...
mod tests {
    ...
    use super::Car;
    use crate::{
        CarBodyConfig, CarControls, MutatorConfig, Team,
        bullet::{
            collision::broadphase::GridBroadphase,
            dynamics::discrete_dynamics_world::DiscreteDynamicsWorld,
        },
        consts::{TICK_TIME, UU_TO_BT, car as car_consts},
    };

    fn airborne_car() -> (Car, DiscreteDynamicsWorld, MutatorConfig) {
        let mutators = MutatorConfig::default();
        ...
        let mut world = DiscreteDynamicsWorld::new(broadphase, mutators.gravity * UU_TO_BT);
        let mut car = Car::new(0, Team::Blue, &mut world, &mutators, CarBodyConfig::OCTANE);
        car.state.phys.rot_mat = Mat3A::IDENTITY;
        car.state.is_on_ground = false;
        car.state.is_flipping = false;
        car.state.has_flipped = false;
        car.state.is_auto_flipping = false;
        (car, world, mutators)
    }

    #[test]
    fn airborne_boost_button_holds_full_throttle_and_paired_accel() {
        let (mut car, mut world, mutators) = airborne_car();
        assert_eq!(
            car_consts::boost::ACCEL_AIR,
            car_consts::boost::ACCEL_GROUND
        );
        assert_eq!(mutators.boost_accel_air, mutators.boost_accel_ground);

        car.state.controls = CarControls::DEFAULT.with_throttle(0.0).with_boost(true);
        car.state.is_boosting = false;
        car.state.boosting_time = 0.0;
        ...
        let forward = Vec3A::X;
        let throttle_impulse =
            forward * 1.0 * (car_consts::drive::THROTTLE_AIR_ACCEL * UU_TO_BT * TICK_TIME);
        let boost_impulse = mutators.boost_accel_ground * forward * (UU_TO_BT * TICK_TIME);
        assert_eq!(
            world.bodies()[idx].accum_lin_vel,
            throttle_impulse + boost_impulse
        );
    }

    #[test]
    fn boost_release_latch_keeps_full_air_throttle() {
        let (mut car, mut world, mutators) = airborne_car();
        car.state.controls = CarControls::DEFAULT.with_throttle(0.0).with_boost(false);
        ...
        car.state.boost = 100.0;
        car.update_boost(&mut world.bodies_mut()[idx], &mutators);
        assert!(car.state.is_boosting);
        assert_eq!(
            world.bodies()[idx].accum_lin_vel,
            mutators.boost_accel_ground * Vec3A::X * (UU_TO_BT * TICK_TIME)
        );
    }
}
